var str_dt = function (e) {
        var t = new Date(e),
            ...
            n = (n %= 12) || 12,
            a = (a = a[1]) < 10 ? "0" + a : a;
        return month = "" + ["Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"][t.getMonth()], day = "" + t.getDate(), year = t.getFullYear(), month.length < 2 && (month = "0" + month), day.length < 2 && (day = "0" + day), [day + " " + month + "," + year + " <small class='text-muted'>" + n + ":" + a + " " + e + "</small>"]
    },
...
// }, xhttp.open("GET", "assets/json/orders-list.init.json"), xhttp.send(), isCount = (new DOMParser).parseFromString(orderList.items.slice(-1)[0]._values.id, "text/html");
    ...
    tabEl.forEach(function (e) {
        ...
        e.relatedTarget.classList.contains("edit-item-btn") ? (document.getElementById("exampleModalLabel").innerHTML = "Edit Order", document.getElementById("showModal").querySelector(".modal-footer").style.display = "block", document.getElementById("add-btn").style.display = "none", document.getElementById("edit-btn").style.display = "block") : e.relatedTarget.classList.contains("add-btn") ? (document.getElementById("modal-id").style.display = "none", document.getElementById("exampleModalLabel").innerHTML = "Add Order", document.getElementById("showModal").querySelector(".modal-footer").style.display = "block", document.getElementById("edit-btn").style.display = "none", document.getElementById("add-btn").style.display = "block") : (document.getElementById("exampleModalLabel").innerHTML = "List Order", document.getElementById("showModal").querySelector(".modal-footer").style.display = "none")
    ...
    addBtn.addEventListener("click", function (e) {
        "" !== customerNameField.value && "" !== productNameField.value && "" !== dateField.value && "" !== amountField.value && "" !== paymentField.value && (orderList.add({
        ...
        document.getElementById("exampleModalLabel").innerHTML = "Edit Order", orderList.get({
            id: idField.value
        }).forEach(function (e) {
    ...
    function refreshCallbacks() {
        removeBtns.forEach(function (e) {
            e.addEventListener("click", function (e) {
                e.target.closest("tr").children[1].innerText, itemId = e.target.closest("tr").children[1].innerText, orderList.get({
                    id: itemId
                }).forEach(function (e) {
                    ...
                    deleteid.body.firstElementChild.innerHTML == itemId && document.getElementById("delete-record").addEventListener("click", function () {
                        orderList.remove("id", t.outerHTML), document.getElementById("deleteOrder").click()
                    })
                })
            })
            ...
            e.addEventListener("click", function (e) {
                e.target.closest("tr").children[1].innerText, itemId = e.target.closest("tr").children[1].innerText, orderList.get({
                    ...
                    t == itemId && (idField.value = t, customerNameField.value = e._values.customer_name, productNameField.value = e._values.product_name, dateField.value = e._values.date, amountField.value = e._values.amount, example && example.destroy(), example = new Choices(paymentField, {
                        searchEnabled: !1
